...
void GameManager::LoadMainScript(std::unique_ptr<Registry>& registry, sol::state& lua)
{
    // Find and load Lua script
    std::string path = SDL_GetBasePath();
    path += "assets\\scripts\\ParticlesTest.lua";
    ...
    // Load entities, entity tags and entity components
    ...
    while (true)
    {
        ...
        sol::optional<sol::table> line = entity["components"]["line"];
        if (line != sol::nullopt)
        {
            newEntity.AddComponent<LineComponent>(
                Vec2(
                    entity["components"]["line"]["p1"]["x"].get_or(0.0),
                    entity["components"]["line"]["p1"]["y"].get_or(0.0)),
                Vec2(
                    entity["components"]["line"]["p2"]["x"].get_or(0.0),
                    entity["components"]["line"]["p2"]["y"].get_or(0.0)));
        }

        sol::optional<sol::table> particle = entity["components"]["particle"];
